    ...
    <>
      ...
      >
        <HeaderFooter className="px-4 py-3">
          <FormHeading>Pause this session</FormHeading>
        </HeaderFooter>

        <DialogContent>
          <DialogContentText>
            Are you sure you want to pause this session ? This will take all
            hosts and speakers on backstage.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <button
          ...
          <button
            ...
            onClick={() => {
              socket.emit("pauseSession", {sessionId}, (error) => {
                console.log(error);
              })
              handleClose();
            }}
