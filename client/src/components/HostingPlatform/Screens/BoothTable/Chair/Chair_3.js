...
const Chair_3 = ({ id, launchTableScreen }) => {
  ...
  useEffect(() => {
    if (userImage3) {
      fetchImage(userImage3, id).catch((error) => {
        console.log(error);
      });
    } else {
  ...
  return (
    <>
      <div
        ...
        id={`${id}_chair_3`}
        onClick={() => {
          dispatch(editCurrentlyJoinedChair(`${id}_chair_3`));
